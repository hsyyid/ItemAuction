	...
	public CommandResult execute(CommandSource src, CommandContext ctx) throws CommandException
	{
		if (src instanceof Player)
		{
			...
			if (auction.isPresent())
			{
				...
				if (bid.isPresent() && player.getItemInHand(HandTypes.MAIN_HAND).isPresent() && 
					player.getItemInHand(HandTypes.MAIN_HAND).get().getQuantity() == auction.get().getItemStack().getQuantity() &&
					player.getItemInHand(HandTypes.MAIN_HAND).get().getItem() == auction.get().getItemStack().getItem())
				{
